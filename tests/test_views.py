...
class ViewsTestFunctor:
    def __init__(self, threads: int, i_1: int, i_2: int, i_3: int, i_4: int):
        ...
        self.subview3D: pk.View3D[pk.int32] = self.altView3D[
            :, : i_2 // 2, i_3 // 2 : i_3
        ]
...
class TestViews(unittest.TestCase):
    def setUp(self):
        ...
        self.functor = ViewsTestFunctor(
            self.threads, self.i_1, self.i_2, self.i_3, self.i_4
        )
        ...
        for i in range(self.i_1):
            for j in range(self.i_2):
                for k in range(self.i_3):
                    ...
                    self.assertEqual(expected_result, self.functor.myView3D[i][j][k])
    ...
    def test_arrays(self):
        ...
        pk.parallel_for(
            pk.RangePolicy(pk.Cuda, 0, self.threads), addition_cp, cp_arr=cp_view
        )
        pk.parallel_for(
            pk.RangePolicy(pk.OpenMP, 0, self.threads), addition_np, np_arr=list_view
        )
    ...
    @pytest.mark.skipif(not HAS_CUDA, reason="CUDA/cupy not available")
    @pytest.mark.xfail(HAS_CUDA, reason="bool not supported with CUDA/GPUs yet")
    def test_real(self):
