...
@pytest.mark.parametrize("alpha, a, b, beta, expected_c", [
    (1.0,
     np.array([[1, 7, 9],
               [3, 1, 3],
...
def test_dgemm_vs_scipy(alpha,
    ...
    # NOTE: it seems that SciPy may ignore beta
    # in its wrapper perhaps?
    # see: https://github.com/scipy/scipy/issues/17594
    # so I've kept beta fixed at 1.0 for now in these
    # tests
...
def test_dgemm_input_handling():
    alpha = 1.0
    view_a = pk.from_numpy(np.zeros((4, 3)))
    view_b = pk.from_numpy(np.zeros((4, 3)))
