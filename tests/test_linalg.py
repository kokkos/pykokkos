...
def test_dgemm_shape(shape_a, shape_b):
    ...
    alpha = 1.0
    actual = dgemm(alpha=alpha, view_a=view_a, view_b=view_b)
    # shape requirement of C
    assert actual.shape == (view_a.shape[0], view_b.shape[1])


@pytest.mark.parametrize(
    "alpha, a, b, c, beta, expected_c",
    [
        (
            1.0,
            np.array([[1, 7, 9], [3, 1, 3], [5, 5, 22]], dtype=np.float64),
            np.array([[9, 0, 2], [77, 100, 4], [1, 500, 9]], dtype=np.float64),
            None,
            1.0,
            np.array(
                [
