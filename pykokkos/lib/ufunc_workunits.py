...
@pk.workunit
def multiply_impl_1d_int8(tid: int, viewA: pk.View1D[pk.int8], viewB: pk.View1D[pk.int8], out: pk.View1D[pk.int8]):
    out[tid] = viewA[tid] * viewB[tid]


@pk.workunit
def multiply_impl_1d_int16(tid: int, viewA: pk.View1D[pk.int16], viewB: pk.View1D[pk.int16], out: pk.View1D[pk.int16]):
    out[tid] = viewA[tid] * viewB[tid]


@pk.workunit
def multiply_impl_1d_int32(tid: int, viewA: pk.View1D[pk.int32], viewB: pk.View1D[pk.int32], out: pk.View1D[pk.int32]):
    out[tid] = viewA[tid] * viewB[tid]


@pk.workunit
def multiply_impl_1d_int64(tid: int, viewA: pk.View1D[pk.int64], viewB: pk.View1D[pk.int64], out: pk.View1D[pk.int64]):
    out[tid] = viewA[tid] * viewB[tid]


@pk.workunit
def multiply_impl_1d_uint8(tid: int, viewA: pk.View1D[pk.uint8], viewB: pk.View1D[pk.uint8], out: pk.View1D[pk.uint8]):
    out[tid] = viewA[tid] * viewB[tid]


@pk.workunit
def multiply_impl_1d_uint16(tid: int, viewA: pk.View1D[pk.uint16], viewB: pk.View1D[pk.uint16], out: pk.View1D[pk.uint16]):
    out[tid] = viewA[tid] * viewB[tid]


@pk.workunit
def multiply_impl_1d_uint32(tid: int, viewA: pk.View1D[pk.uint32], viewB: pk.View1D[pk.uint32], out: pk.View1D[pk.uint32]):
    out[tid] = viewA[tid] * viewB[tid]


@pk.workunit
def multiply_impl_1d_uint64(tid: int, viewA: pk.View1D[pk.uint64], viewB: pk.View1D[pk.uint64], out: pk.View1D[pk.uint64]):
    out[tid] = viewA[tid] * viewB[tid]


@pk.workunit
def multiply_impl_2d_double(tid: int, viewA: pk.View2D[pk.double], viewB: pk.View2D[pk.double], out: pk.View2D[pk.double]):
    for i in range(viewA.extent(1)):
        out[tid][i] = viewA[tid][i] * viewB[tid][i]


@pk.workunit
def multiply_impl_2d_uint8(tid: int, viewA: pk.View2D[pk.uint8], viewB: pk.View2D[pk.uint8], out: pk.View2D[pk.uint8]):
    for i in range(viewA.extent(1)):
        out[tid][i] = viewA[tid][i] * viewB[tid][i]


@pk.workunit
def multiply_impl_2d_float(tid: int, viewA: pk.View2D[pk.float], viewB: pk.View2D[pk.float], out: pk.View2D[pk.float]):
    for i in range(viewA.extent(1)):
        out[tid][i] = viewA[tid][i] * viewB[tid][i]


@pk.workunit
def multiply_impl_2d_int8(tid: int, viewA: pk.View2D[pk.int8], viewB: pk.View2D[pk.int8], out: pk.View2D[pk.int8]):
    for i in range(viewA.extent(1)):
        out[tid][i] = viewA[tid][i] * viewB[tid][i]


@pk.workunit
def multiply_impl_2d_int16(tid: int, viewA: pk.View2D[pk.int16], viewB: pk.View2D[pk.int16], out: pk.View2D[pk.int16]):
    for i in range(viewA.extent(1)):
        out[tid][i] = viewA[tid][i] * viewB[tid][i]


@pk.workunit
def multiply_impl_2d_int32(tid: int, viewA: pk.View2D[pk.int32], viewB: pk.View2D[pk.int32], out: pk.View2D[pk.int32]):
    for i in range(viewA.extent(1)):
        out[tid][i] = viewA[tid][i] * viewB[tid][i]


@pk.workunit
def multiply_impl_2d_int64(tid: int, viewA: pk.View2D[pk.int64], viewB: pk.View2D[pk.int64], out: pk.View2D[pk.int64]):
    for i in range(viewA.extent(1)):
        out[tid][i] = viewA[tid][i] * viewB[tid][i]


@pk.workunit
def multiply_impl_3d_float(tid: int, viewA: pk.View3D[pk.float], viewB: pk.View3D[pk.float], out: pk.View3D[pk.float]):
    for i in range(viewA.extent(1)):
...
@pk.workunit
def multiply_impl_3d_double(tid: int, viewA: pk.View3D[pk.double], viewB: pk.View3D[pk.double], out: pk.View3D[pk.double]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            out[tid][i][j] = viewA[tid][i][j] * viewB[tid][i][j]
...
@pk.workunit
def multiply_impl_3d_uint8(tid: int, viewA: pk.View3D[pk.uint8], viewB: pk.View3D[pk.uint8], out: pk.View3D[pk.uint8]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            out[tid][i][j] = viewA[tid][i][j] * viewB[tid][i][j]
...
@pk.workunit
def multiply_impl_3d_uint16(tid: int, viewA: pk.View3D[pk.uint16], viewB: pk.View3D[pk.uint16], out: pk.View3D[pk.uint16]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            out[tid][i][j] = viewA[tid][i][j] * viewB[tid][i][j]
...
@pk.workunit
def multiply_impl_3d_uint32(tid: int, viewA: pk.View3D[pk.uint32], viewB: pk.View3D[pk.uint32], out: pk.View3D[pk.uint32]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            out[tid][i][j] = viewA[tid][i][j] * viewB[tid][i][j]
...
@pk.workunit
def multiply_impl_3d_uint64(tid: int, viewA: pk.View3D[pk.uint64], viewB: pk.View3D[pk.uint64], out: pk.View3D[pk.uint64]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            out[tid][i][j] = viewA[tid][i][j] * viewB[tid][i][j]

@pk.workunit
def multiply_impl_3d_int8(tid: int, viewA: pk.View3D[pk.int8], viewB: pk.View3D[pk.int8], out: pk.View3D[pk.int8]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            out[tid][i][j] = viewA[tid][i][j] * viewB[tid][i][j]
...
@pk.workunit
def multiply_impl_3d_int16(tid: int, viewA: pk.View3D[pk.int16], viewB: pk.View3D[pk.int16], out: pk.View3D[pk.int16]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            out[tid][i][j] = viewA[tid][i][j] * viewB[tid][i][j]
...
@pk.workunit
def multiply_impl_3d_int32(tid: int, viewA: pk.View3D[pk.int32], viewB: pk.View3D[pk.int32], out: pk.View3D[pk.int32]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            out[tid][i][j] = viewA[tid][i][j] * viewB[tid][i][j]
...
@pk.workunit
def multiply_impl_3d_int64(tid: int, viewA: pk.View3D[pk.int64], viewB: pk.View3D[pk.int64], out: pk.View3D[pk.int64]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            out[tid][i][j] = viewA[tid][i][j] * viewB[tid][i][j]
...
@pk.workunit
def multiply_impl_4d_float(tid: int, viewA: pk.View4D[pk.float], viewB: pk.View4D[pk.float], out: pk.View4D[pk.float]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
...
@pk.workunit
def multiply_impl_4d_double(tid: int, viewA: pk.View4D[pk.double], viewB: pk.View4D[pk.double], out: pk.View4D[pk.double]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                out[tid][i][j][k] = viewA[tid][i][j][k] * viewB[tid][i][j][k]
...
@pk.workunit
def multiply_impl_4d_uint8(tid: int, viewA: pk.View4D[pk.uint8], viewB: pk.View4D[pk.uint8], out: pk.View4D[pk.uint8]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                out[tid][i][j][k] = viewA[tid][i][j][k] * viewB[tid][i][j][k]
...
@pk.workunit
def multiply_impl_4d_uint16(tid: int, viewA: pk.View4D[pk.uint16], viewB: pk.View4D[pk.uint16], out: pk.View4D[pk.uint16]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                out[tid][i][j][k] = viewA[tid][i][j][k] * viewB[tid][i][j][k]
...
@pk.workunit
def multiply_impl_4d_uint32(tid: int, viewA: pk.View4D[pk.uint32], viewB: pk.View4D[pk.uint32], out: pk.View4D[pk.uint32]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                out[tid][i][j][k] = viewA[tid][i][j][k] * viewB[tid][i][j][k]
...
@pk.workunit
def multiply_impl_4d_uint64(tid: int, viewA: pk.View4D[pk.uint64], viewB: pk.View4D[pk.uint64], out: pk.View4D[pk.uint64]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                out[tid][i][j][k] = viewA[tid][i][j][k] * viewB[tid][i][j][k]
...
@pk.workunit
def multiply_impl_4d_int8(tid: int, viewA: pk.View4D[pk.int8], viewB: pk.View4D[pk.int8], out: pk.View4D[pk.int8]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                out[tid][i][j][k] = viewA[tid][i][j][k] * viewB[tid][i][j][k]
...
@pk.workunit
def multiply_impl_4d_int16(tid: int, viewA: pk.View4D[pk.int16], viewB: pk.View4D[pk.int16], out: pk.View4D[pk.int16]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                out[tid][i][j][k] = viewA[tid][i][j][k] * viewB[tid][i][j][k]
...
@pk.workunit
def multiply_impl_4d_int32(tid: int, viewA: pk.View4D[pk.int32], viewB: pk.View4D[pk.int32], out: pk.View4D[pk.int32]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                out[tid][i][j][k] = viewA[tid][i][j][k] * viewB[tid][i][j][k]
...
@pk.workunit
def multiply_impl_4d_int64(tid: int, viewA: pk.View4D[pk.int64], viewB: pk.View4D[pk.int64], out: pk.View4D[pk.int64]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                out[tid][i][j][k] = viewA[tid][i][j][k] * viewB[tid][i][j][k]
...
@pk.workunit
def multiply_impl_5d_float(tid: int, viewA: pk.View5D[pk.float], viewB: pk.View5D[pk.float], out: pk.View5D[pk.float]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
...
@pk.workunit
def multiply_impl_5d_double(tid: int, viewA: pk.View5D[pk.double], viewB: pk.View5D[pk.double], out: pk.View5D[pk.double]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                for l in range(viewA.extent(4)):
...
@pk.workunit
def multiply_impl_5d_uint8(tid: int, viewA: pk.View5D[pk.uint8], viewB: pk.View5D[pk.uint8], out: pk.View5D[pk.uint8]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                for l in range(viewA.extent(4)):
...
@pk.workunit
def multiply_impl_5d_uint16(tid: int, viewA: pk.View5D[pk.uint16], viewB: pk.View5D[pk.uint16], out: pk.View5D[pk.uint16]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                for l in range(viewA.extent(4)):
...
@pk.workunit
def multiply_impl_5d_uint32(tid: int, viewA: pk.View5D[pk.uint32], viewB: pk.View5D[pk.uint32], out: pk.View5D[pk.uint32]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                for l in range(viewA.extent(4)):
...
@pk.workunit
def multiply_impl_5d_uint64(tid: int, viewA: pk.View5D[pk.uint64], viewB: pk.View5D[pk.uint64], out: pk.View5D[pk.uint64]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                for l in range(viewA.extent(4)):
...
@pk.workunit
def multiply_impl_5d_int8(tid: int, viewA: pk.View5D[pk.int8], viewB: pk.View5D[pk.int8], out: pk.View5D[pk.int8]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                for l in range(viewA.extent(4)):
...
@pk.workunit
def multiply_impl_5d_int16(tid: int, viewA: pk.View5D[pk.int16], viewB: pk.View5D[pk.int16], out: pk.View5D[pk.int16]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                for l in range(viewA.extent(4)):
...
@pk.workunit
def multiply_impl_5d_int32(tid: int, viewA: pk.View5D[pk.int32], viewB: pk.View5D[pk.int32], out: pk.View5D[pk.int32]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                for l in range(viewA.extent(4)):
...
@pk.workunit
def multiply_impl_5d_int64(tid: int, viewA: pk.View5D[pk.int64], viewB: pk.View5D[pk.int64], out: pk.View5D[pk.int64]):
    for i in range(viewA.extent(1)):
        for j in range(viewA.extent(2)):
            for k in range(viewA.extent(3)):
                for l in range(viewA.extent(4)):
...
@pk.workunit
def multiply_impl_2d_uint16(tid: int, viewA: pk.View2D[pk.uint16], viewB: pk.View2D[pk.uint16], out: pk.View2D[pk.uint16]):
    for i in range(viewA.extent(1)):
        out[tid][i] = viewA[tid][i] * viewB[tid][i]


@pk.workunit
def multiply_impl_2d_uint32(tid: int, viewA: pk.View2D[pk.uint32], viewB: pk.View2D[pk.uint32], out: pk.View2D[pk.uint32]):
    for i in range(viewA.extent(1)):
        out[tid][i] = viewA[tid][i] * viewB[tid][i]


@pk.workunit
def multiply_impl_2d_uint64(tid: int, viewA: pk.View2D[pk.uint64], viewB: pk.View2D[pk.uint64], out: pk.View2D[pk.uint64]):
    for i in range(viewA.extent(1)):
        out[tid][i] = viewA[tid][i] * viewB[tid][i]
