...
def ravel(view, order="C"):
    if view.rank() == 2:
        if view.dtype.__name__ == "float64":
            ...
            if order == "F":
                pk.parallel_for(
                    view.shape[1], ravel_F_impl_2d_double, view=view, out=out
                )
            elif order == "C":
                pass
                pk.parallel_for(
                    view.shape[0], ravel_C_impl_2d_double, view=view, out=out
                )
            return out
...
@pk.workunit
def expand_dims_0_impl_double(
    tid: int, view: pk.View1D[pk.double], out: pk.View2D[pk.double]
):
    out[0][tid] = view[tid]


@pk.workunit
def expand_dims_1_impl_double(
    tid: int, view: pk.View1D[pk.double], out: pk.View2D[pk.double]
):
    out[tid][0] = view[tid]
...
@pk.workunit
def expand_dims_0_impl_2d_double(
    tid: int, view: pk.View2D[pk.double], out: pk.View3D[pk.double]
):
    for i in range(view.extent(1)):
...
@pk.workunit
def expand_dims_1_impl_2d_double(
    tid: int, view: pk.View2D[pk.double], out: pk.View3D[pk.double]
):
    for i in range(view.extent(1)):
...
def expand_dims(view, axis=0):
    ...
    if view.rank() == 1:
        if axis == 0:
            ...
            pk.parallel_for(
                view.shape[0], expand_dims_0_impl_double, view=view, out=out
            )
        else:
            out = pk.View([*view.shape, 1], pk.double)
            pk.parallel_for(
                view.shape[0], expand_dims_1_impl_double, view=view, out=out
            )
    ...
    if view.rank() == 2:
        if axis == 0:
            out = pk.View([1, *view.shape], pk.double)
            pk.parallel_for(
                view.shape[0], expand_dims_0_impl_2d_double, view=view, out=out
            )
        else:
            out = pk.View([view.shape[0], 1, view.shape[1]], pk.double)
            pk.parallel_for(
                view.shape[0], expand_dims_1_impl_2d_double, view=view, out=out
            )
