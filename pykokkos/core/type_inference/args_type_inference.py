...
class UpdatedTypes:
    ...
    inferred_types: Dict[
        str, str
    ]  # type information stored as string: identifier -> type
    param_list: List[str]
...
class UpdatedDecorator:
    ...
    inferred_decorator: Dict[
        str, Dict[str, str]
    ]  # against each view (first dict) values for layout, space, and trait
    param_list: List[
        str
    ]  # Original params needed to reset AST incase user provides all annotations
...
def get_annotations(
    parallel_type: str,
    workunit_trees: Union[Tuple[Callable, ast.AST], List[Tuple[Callable, ast.AST]]],
    policy: Union[ExecutionPolicy, int],
    passed_kwargs,
) -> Optional[UpdatedTypes]:
    """
    ...
    if isinstance(workunit_trees, list):
        ...
        passed_kwargs, param_list = fuse_workunit_kwargs_and_params(
            trees, passed_kwargs, parallel_type
        )
    else:
    ...
    updated_types = infer_policy_args(
        param_list, policy_params, policy, parallel_type, updated_types
    )

    # Policy parameters are the only parameters
    if not len(passed_kwargs):
        if not len(updated_types.inferred_types):
            return None
...
def get_views_decorator(
    parallel_type: str, workunit_trees: List[Tuple[Callable, ast.AST]], passed_kwargs
) -> UpdatedDecorator:
    """
    ...
    updated_decorator = UpdatedDecorator(inferred_decorator={}, param_list=param_list)
    ...
    for kwarg in passed_kwargs:
        ...
        updated_decorator.inferred_decorator[kwarg]["trait"] = str(value.trait).split(
            "."
        )[1]
        updated_decorator.inferred_decorator[kwarg]["layout"] = str(value.layout).split(
            "."
        )[1]
        updated_decorator.inferred_decorator[kwarg]["space"] = str(value.space).split(
            "."
        )[1]
...
def infer_policy_args(
    ...
    for i in range(policy_params):
        ...
        if (
            i == policy_params - 1
            and parallel_type == "parallel_reduce"
            or i == policy_params - 2
            and parallel_type == "parallel_scan"
        ):
...
def infer_other_args(
    param_list: List[inspect.Parameter], passed_kwargs, updated_types: UpdatedTypes
) -> UpdatedTypes:
    """
...
def get_type_info(
    ...
    for this_workunit, this_parser in zip(workunit, parser):
        ...
        is_missing_annotations: bool = (
            ...
            or check_missing_annotations(this_tree.args.args)
        )
    ...
    workunit_trees, workunit, entity_AST = prepare_fusion_args(
        list_passed, workunit, entity_AST
    )
    ...
    if is_standalone_workunit:
        updated_types = get_annotations(
            f"parallel_{operation}", workunit_trees, policy, passed_kwargs
        )
        updated_decorator = get_views_decorator(
            f"parallel_{operation}", workunit_trees, passed_kwargs
        )
        types_signature = get_types_signature(
            updated_types, updated_decorator, execution_space
        )
...
def prepare_fusion_args(
    list_passed: bool, workunit: List[Callable], entity_AST: List[ast.AST]
) -> Tuple[
