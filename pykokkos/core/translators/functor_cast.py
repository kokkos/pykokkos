...
from .bindings import (
    generate_functor_instance,
    generate_copy_back,
    get_view_memory_space,
    generate_copy_back_from_dict,
)


def generate_cast_from_object(
    functor: str, members: PyKokkosMembers, precision: Optional[DataType]
) -> str:
    """
    ...
    for name, param_type in params.items():
        if param_type == "const std::string&":
            cast_source += (
                f'std::string {name} = getattr(obj,"{name}").cast<std::string>();'
            )
        else:
            cast_source += (
                f'{param_type} {name} = getattr(obj,"{name}").cast<{param_type}>();'
            )

    cast_source += generate_functor_instance(
        functor_type, members, False, "ExecSpace", True
    )
...
def generate_cast_to_object(
    ...
    if members.has_real:
        ...
        template_decl = (
            f"template <class ExecSpace,class ArgumentMemorySpace, class {real_str}>"
        )
    else:
        functor_type = f"{functor}<ExecSpace>"
        template_decl = f"template <class ExecSpace,class ArgumentMemorySpace>"
    ...
    deep_copy_args: Dict[str, str] = {
        v.declname: f"functor.{v.declname}"
