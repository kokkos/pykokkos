...
@pk.workunit
def dgemm_impl_tiled_no_view_c(
    ...
    scratch_mem_a: pk.ScratchView1D[float] = pk.ScratchView1D(
        team_member.team_scratch(0), team_member.team_size()
    )
    scratch_mem_b: pk.ScratchView1D[float] = pk.ScratchView1D(
        team_member.team_scratch(0), team_member.team_size()
    )
    ...
    for row_factor in range(0, width, slide_size):
        for col_factor in range(0, width, slide_size):
            ...
            for i in range(width / 2):
                scratch_mem_a[team_member.team_rank()] = view_a[row + row_factor][
                    i * 2 + ty
                ]
                scratch_mem_b[team_member.team_rank()] = view_b[i * 2 + tx][
                    col + col_factor
                ]
