...
if __name__ == "__main__":
    ...
    for system_size in ["small", "medium", "large"]:
        ...
        print(
            f"PyKokkos DGEMM execution time (s) for {num_repeats} repeats: {pk_dgemm_time_sec}"
        )
        scipy_dgemm_time_sec = timeit.timeit(
            "scipy_dgemm(alpha, a_new, b_new, beta, c_new)",
            globals=globals(),
            number=num_repeats,
        )
        results["SciPy"][system_size] = scipy_dgemm_time_sec
        print(
            f"SciPy DGEMM execution time (s) for {num_repeats} repeats: {scipy_dgemm_time_sec}"
        )
        ...
        if ratio == 1:
            ...
            print(
                f"PyKokkos DGEMM timing is slower than SciPy with ratio: {ratio:.2f} fold"
            )
        else:
            print(
                f"PyKokkos DGEMM timing is faster than SciPy with ratio: {ratio:.2f} fold"
            )
        print("-" * 20)
