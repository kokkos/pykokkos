...
def force_modules_instantiation(
    exa_input: Input, system: System, binning: Binning
) -> Force:
    half_neigh: bool = (
        exa_input.force_iteration_type == ForceIterationType.FORCE_ITER_NEIGH_HALF.value
    )
    if exa_input.force_type == ForceType.FORCE_LJ.value:
        if exa_input.neighbor_type == NeighborType.NEIGH_2D.value:
            return ForceLJNeigh(
                exa_input.input_data.words[exa_input.force_line], system, half_neigh
            )
