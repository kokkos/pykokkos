...
class main:
    def __init__(self, iterations, n, tile_size, star, radius):
        ...
        self.inp: pk.View2D[pk.double] = pk.View(
            [self.n, self.n], pk.double, layout=pk.Layout.LayoutRight
        )
        self.out: pk.View2D[pk.double] = pk.View(
            [self.n, self.n], pk.double, layout=pk.Layout.LayoutRight
        )
        self.norm: float = 0
    ...
    def run(self):
        ...
        for i in range(iterations):
            ...
            if r == 1:
                ...
                pk.parallel_for(
                    "stencil",
                    pk.MDRangePolicy([r, r], [n - r, n - r], [t, t]),
                    self.star1,
                )
            elif r == 2:
                # star2 stencil
            ...
            pk.parallel_for(pk.MDRangePolicy([0, 0], [n, n], [t, t]), self.increment)
        ...
        if abs(self.norm - reference_norm) > episilon:
            pk.printf(
                "ERROR: L1 norm != Reference norm err=%.2f\n",
